...
for indID = 1:length(cellIDs), % process each cell type
  ...
  % Read model OCV file
  modelFile = sprintf('../data/%smodel-ocv.mat',cellID);
  load(modelFile);

  % Read MAT raw data files
  data = zeros([0 length(mags{indID} > 0)]);
  dataInd = 0;
  for indTemps = 1:length(mags{indID}),
    ...
    if temps(indTemps) < 0,
      DYNPrefix = sprintf('../data/%s_DYN/%s_DYN_%02d_N%02d',...
        cellID,cellID,theMag,abs(temps(indTemps)));
    else
      DYNPrefix = sprintf('../data/%s_DYN/%s_DYN_%02d_P%02d',...
